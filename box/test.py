...
from box.ic import ic         # Multi-Use
import time
...
def insert_dictionaries_test():
    from box.db import insert_dictionaries
    cursor, connection = sqlite_init()

    path = r"C:\Users\vivek\Desktop\bkp\game"
    status = "CLEAN"
    url = "https://github.com/box/bkp_game"
    order = 0
...
def multiprocessing_multithreading_kill_test():
    ...
    from box.error import print_error_information
...
def test_partition():
    from box.common import create_partitions
...
def test_trace():
    from box.trace import trace
    from box.trace import rerun
...
def test_color_bash():
    ...
    SELECT * FROM items
    """
    print("".join([Code.LIGHTGREEN_EX + "box@vivobook", ":", Code.LIGHTBLUE_EX + "/mnt/c/Users/vivek/Documents/Python"]))

def test_yield_list(): 

    def db_gen(query):
        from box.reltools import rel2abs
        from box.db import db_init
...
def test_summaries():
    from box.common import Date
    from box.ic import ic; ic
    from datetime import datetime
    dt = datetime.now()
    year = dt.year
